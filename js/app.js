...
const App = {
    ...
    loadAdminDashboard() {
        ...
        const renderBoth = () => {
            document.getElementById('app').innerHTML = renderAdminDashboard(AppState.user, pending, completed, professionals);
        ...
        const unsub1 = listenForPendingPayments((data) => {
            pending = data;
            renderBoth();
        });
        const unsub2 = listenForCompletedServices((data) => {
            completed = data;
            renderBoth();
        });

        // Cargar profesionales para validacion
        getProfessionalsForAdmin().then(data => {
            professionals = data;
            renderBoth();
        });

        AppState.adminListeners.push(unsub1, unsub2);
    },
    ...
    bindAdminEvents() {
        ...
        document.querySelectorAll('.btn-approve-pay').forEach(btn => {
            btn.addEventListener('click', async (e) => {
                ...
                const confirmToApprove = confirm("¿El voucher y monto son correctos? Al aprobar, el especialista será notificado para iniciar el servicio.");
                if (!confirmToApprove) return;

                const oldHtml = e.currentTarget.innerHTML;
                e.currentTarget.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i>';
                e.currentTarget.disabled = true;

                // Also disable reject button next to it
        ...
        document.querySelectorAll('.btn-reject-pay').forEach(btn => {
            btn.addEventListener('click', async (e) => {
                ...
                const confirmToReject = confirm("¿El voucher es inválido? Al rechazar, se le pedirá al cliente que suba otro voucher.");
                if (!confirmToReject) return;

                const oldHtml = e.currentTarget.innerHTML;
                e.currentTarget.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i>';
                e.currentTarget.disabled = true;

                // Also disable approve button previous to it
                const approveBtn = e.currentTarget.previousElementSibling;
        ...
        document.querySelectorAll('.btn-mark-paid').forEach(btn => {
            btn.addEventListener('click', async (e) => {
                const reqId = e.currentTarget.dataset.id;
                const confirmToPay = confirm("¿Confirmas que ya hiciste la transferencia bancaria al profesional?");
                if (!confirmToPay) return;
        ...
        document.querySelectorAll('.btn-approve-prof').forEach(btn => {
            btn.addEventListener('click', async (e) => {
                ...
                const res = await updateProfessionalStatus(uid, 'aprobada');
                if (res.success) {
        ...
        // Observe Professional Document Verification
        document.querySelectorAll('.btn-observe-prof').forEach(btn => {
            btn.addEventListener('click', async (e) => {
                const uid = e.currentTarget.dataset.id;

                const reason = prompt("Indica el motivo de la observación (ej: DNI borroso, faltan certificados):");
                if (!reason) return; // Si cancela o deja vacío, abortar

                const oldHtml = e.currentTarget.innerHTML;
                e.currentTarget.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i>';
                e.currentTarget.disabled = true;

                const res = await updateProfessionalStatus(uid, 'observada', reason);
                if (res.success) {
                    // Recargar el dashboard
                    this.loadAdminDashboard();
                } else {
